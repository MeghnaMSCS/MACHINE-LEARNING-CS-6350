...
classifier_p = Perceptron(shuffle=True)
classifier_p.fit(trainData.drop(['income>50K'],axis=1),trainData['income>50K'])
output_p=classifier_p.predict(testData.drop(['ID'],axis=1))
...
output_data_col = {'ID':ID,"Prediction":output_p}
output_data=pd.DataFrame(output_data_col)
output_data.to_csv("MLsub_Perceptron.csv",index=False)
